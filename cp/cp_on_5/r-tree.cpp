    ...
    void printTree(Node* node, int depth = 0) {
        ...
        if (node->isLeaf) {
            ...
            for (size_t i = 0; i < node->entries.size(); ++i) {
                std::cout << indent << "  MBR: (" << node->entries[i].lowLeft.x << ", " << node->entries[i].lowLeft.y << ") - (" 
                        << node->entries[i].upRight.x << ", " << node->entries[i].upRight.y << ") Id: " 
                        << node->indices[i] << std::endl;
            }
        } else {
            std::cout << indent << "Node:" << std::endl;
            for (size_t i = 0; i < node->entries.size(); ++i) {
                std::cout << indent << "  MBR: (" << node->entries[i].lowLeft.x << ", " << node->entries[i].lowLeft.y << ") - ("
                        << node->entries[i].upRight.x << ", " << node->entries[i].upRight.y << ")" << std::endl;
            }
...
int main() {
    ...
    for (const auto& p : points) {
        ...
        if (!containingPolygons.empty()) {
            for (int id : containingPolygons) {
                std::cout << id << " ";
            }
        } else {
            std::cout << -1;
